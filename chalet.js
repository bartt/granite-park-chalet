...
var PREV_AVAILABILITY_FILE = process.env.PREV_AVAILABILITY_FILE
  || './prev-availability.json';

function save(availableDays) {
  fs.writeFile(PREV_AVAILABILITY_FILE, JSON.stringify(availableDays), 'utf8',
    function (err) {
      if (err) {
        debug(err);
        throw err;
...
function notify(availableDays) {
  var availabilityText = availableDays.length ? availableDays.join(', ')
    : 'No availability';
